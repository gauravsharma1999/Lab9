...
public class ElectricPokemon extends Pokemon {
    ...
    /**
     * Our ElectricPokemon constructor. Constructs a new ElectricPokemon with the Pokemon Type of
     * electric. ElectricPokemon's specialty attack is THUNDERBOLT
     */
    ...
    /**
     * We have to modify our attack slightly to account for our new specialty attacks. We will first
     * have a normal attack that we're used to with our normal Pokemon. Then, if the normal attack
     * wasn't already fatal, we check to see if the Pokemon can execute a fatal specialty attack to
     ...
     * - The probability of the specialty attack is greater than the next random value <br>
     ...
     * Example of a specialty attack hitting: Pikachu is attacking Bubba <br>
     ...
     * @return whether or not the game has ended Implement this.
     */
    public boolean attack(final Pokemon opponent) {
        super.attack(opponent);
        if (opponent.getHitPoints() > 0 && opponent.pokeType != this.pokeType) {
            double prob = Math.random();
            if (specProb > prob) {
                System.out.println(
                        opponent.getName() + "executes a special attack... " + specialtyAttack);
